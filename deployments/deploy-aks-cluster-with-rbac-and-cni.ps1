...
az role assignment create --role "Azure Kubernetes Service RBAC Admin" --assignee $AD_ADMIN_GROUP_OBJECT_ID --scope $AKS_ID
az role assignment create --role "Azure Kubernetes Service Cluster User Role" --assignee $AD_API_GROUP_OBJECT_ID --scope $AKS_ID
az role assignment create --role "Azure Kubernetes Service Cluster User Role" --assignee $AD_WEB_GROUP_OBJECT_ID --scope $AKS_ID
...
Write-Host 'Fill in AD_API_DEV_GROUP_OBJECT_ID for role binding:' + $AD_API_GROUP_OBJECT_ID;
Write-Host 'Fill in AD_WEB_DEV_GROUP_OBJECT_ID for role binding:' + $AD_WEB_GROUP_OBJECT_ID;
Write-Host '(Press any key to continue)';
$null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown');
